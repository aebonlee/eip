...
export default function Header() {
  ...
  const isActive = (menuItem: MegaMenuItem) => {
    if (menuItem.path) {
      return location.pathname.startsWith(menuItem.path.split('#')[0])
    }
    if (menuItem.sections) {
      return menuItem.sections.some((s: MenuSection) =>
        s.items.some((item: SectionItem) => {
    ...
    if (menuItem.submenu) {
      return menuItem.submenu.some((item: SubMenuItem) => location.pathname.startsWith(item.path.split('#')[0]))
    }
    return false
  }
