...
const trendLabels = { rising: '상승 추세', falling: '하락 추세', stable: '유지 중', insufficient: '데이터 부족' }
const trendColors = { rising: 'var(--success)', falling: 'var(--error)', stable: 'var(--primary)', insufficient: 'var(--text-light)' }

export default function PassPrediction({ certType }) {
  const { user } = useAuth()
  ...
  if (results.length > 0) {
    ...
    if (latestWithScores?.subject_scores) {
      ...
      Object.entries(ss).forEach(([key, val]) => {
        ...
        if (pct < 60) weaknesses.push({ key, pct, ...val })
      })
  ...
  // 과목명 매핑
  const subjectNames = {}
  if (cert) cert.subjects.forEach(s => { subjectNames[s.id] = s.name })
  const typeNames = { sql: 'SQL', algorithm: '알고리즘', short: '단답형' }
