...
export type CertTypeKey = keyof typeof certTypes

// 모든 챕터를 하나로 통합
export const chapters: Record<string, { id: string; name: string; topics?: string[]; content?: string }[]> = {
  ...engineerChapters,
  ...industrialChapters,
  ...functionalChapters,
}
...
// CBT 모의시험 문제
export const mockQuestions: Record<string, { id: number; subject: string; question: string; options: string[]; answer: number; explanation: string }[]> = {
  engineer: engineerQuestions,
  industrial: industrialQuestions,
  functional: functionalQuestions,
}

export function getQuestionsBySubject(certType: string, subjectId: string) {
  const questions = mockQuestions[certType] || []
  return questions.filter(q => q.subject === subjectId)
}

export function getMockTestQuestions(certType: string, count = 20) {
  const questions = mockQuestions[certType] || []
...
// 회차별 모의시험
export function getMaxWrittenRounds(certType: string) {
  const total = (mockQuestions[certType] || []).length
  return Math.floor(total / 20)
}

export function getWrittenRoundQuestions(certType: string, roundNumber: number, count = 20) {
  const questions = mockQuestions[certType] || []
